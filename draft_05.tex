...
\caption{Breathing mode frequency measured at different $E$ and $F_0$. Inset: a demonstration of the
  oscillation of $E_{trap}$, the total potential energy of particles in the trap, Since
  $E_{trap}\propto \sum_{is}{x_i^2}$, it manifests the oscillation behavior of the radius $R(t)$ as
  defined in Eq.~\ref{eq:def_of_R}.  The breathing frequency is obtained from the Fourier transform
  of $R(t)$.}
...
$R$), so that the frequency of $R(t)$ is $\omega_0=2$.  When there is interaction, the frequency
will get shifted: $\omega_0 = 2+\delta$.  We measure the radius of the cloud $R(t)$ and get the
