...
\paragraph{relation between Radius and Energy}
radius R is defined as the mean-square-root of each particles position $\left\lbrace x_i\right\rbrace$. When E is very large, the interaction energy can be omitted, so E is dominated by the kinetic energy and potential energy of harmonic oscillators. Thus, $E=2*\frac{1}{2}\omega_0^2*\sum_{i}x_i^2=N\omega_0^2R^2$ 
\begin{figure}[hbtp]
\centering
\includegraphics[scale=0.5]{D:/OriginUserfiles/MPI_internship/E_R_fitting.jpg}
-\caption{E-R} This picture shows $E=kR^2$ when energy is large. ($k=N\omega_0^2$)
\end{figure}

\paragraph{Ground states}
For the ground state of this system, one could predict that \\(1) When the sigma is larger than the size of the cloud (in view of the scaling behaviour, large $\sigma$ is equivalent to \textbf{small F0 limit} when we fix $\sigma=1$), it can be easily shown that the ground state is crystal-like, since all every particle keep a equal distance with its nearest neighbours, which is $\frac{2F0}{\omega_0^2}$. The low-lying excitation is particles' independent harmonic oscillation around their equilibrium position.\\(2) When \textbf{F0 is very large} the molecules act like a hardcore gas, while the size of particle is just $\sigma$.

In the picture below, you can see this crossover from the ``solid-like" limit to the ``hardcore gas" limit.

\begin{figure}[hbtp]
\centering
\includegraphics[scale=0.3]{D:/matlab2016/MolecularDynamics/figure/groundstate1.eps}
...
\begin{figure}[hbtp]
\center
\includegraphics[scale=0.8]{D:/matlab2016/MolecularDynamics/figure/latexpic/freq_scanF_differentE_log.eps}
...
\begin{figure}[hbtp]
\centering
\includegraphics[scale=0.4]{D:/matlab2016/MolecularDynamics/figure/latexpic/rotatingframe_example2.eps} 
\includegraphics[scale=0.4]{D:/matlab2016/MolecularDynamics/figure/latexpic/rotatingframe_example1.eps} 
...
\begin{figure}[hbtp]
\centering
\includegraphics[scale=0.8]{D:/matlab2016/MolecularDynamics/figure/latexpic/LLEdistribution_scanE_9_22.eps}
