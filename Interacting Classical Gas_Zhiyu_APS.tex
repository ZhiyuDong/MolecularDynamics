...
\subsection{Bouncing and Passing} 
Back to two-particle case, we notice that there are two dynamically distinct regime: \\1) when the maximum of interaction energy is smaller than internal energy, two particle pass each other every time they meet;\\2) when the maximum of interaction energy is higher than internal energy, two particle bounce back.

We call them passing and bouncing regime respectively. If we estimate internal energy of a pair with E/N, the criterion between these two regime is $\frac{NF_0\sigma}{E}=1 $,which is just the thermalization threshold! Naturally one will ask whether thermalization is related to passing and bouncing. As is shown in last two subsections, in passing regime, the situation can be described by the long-term beat behaviour, which obstruct the thermalization. So in this subsection, we focus on the bouncing regime.

To see how the bouncing process thermalize the system, let's consider the strong interaction limit in bouncing regime: $F_0\rightarrow \infty$. In that case, the bouncing happens in a moment. The picture is completely the same as what we have discussed in section ''Breathing Frequency": One can relabel two particles after the bounce and describe the collision as ''particle A comes from the left and jump by $\sigma$ to the right while particle B comes from the right and jump by $\sigma$ to the left". Now if we consider the energy transfer in this process, the only term changed is the potential term. The magnitude of energy transfer in a collision is $m\omega_0^2x_{coll}\sigma$ (suppose $\sigma<<x_{coll}$), where $x_{coll}$ is the position where the collision occur, and is, generally speaking, proportional to cloud radius R. 
 
In bouncing regime, when F0 increase the collision time $\tau$ decrease to zero, but thanks to the finite interaction range, the energy transfer rate stay finite, which is essentially different from the case in passing regime, where when the F0 decrease to zero, since interaction vanishes the energy transfer rate definitely vanishes.

There is one last concern: as is shown in fig.\ref{fig:thermalization3}, the frequency of two-body motion can go to 2 in bouncing regime. Will there be a long-term beat effect as described in passing regime? The long-term beat may occur when $\omega_r$ is close to 2, which means the collision time $\tau$ compensate the time saved by jumping for $\sigma$. In the language of energy transfer, particle pick up some energy since the trap force act on it for time $\tau$ and lose some energy because leaping $\sigma$ in the trap. The mechanism is similar to that in passing regime. Thus it seems that it is possible to form a long-term beat. However, one should bear in mind that in more-particle case, other particles will strongly interfere the motion of the two we are analysing. It is quite different from long-term beat in small F0 limit. In that limit, if pairs with smallest internal energy form a long-term beat, one can deduce that other particles act like transparent to them. But in bouncing regime, long-term beat is like a ``coincidence"-- it only occurs when one pair's internal energy and F0 is in certain relation, so one pair forming a beat does not ensures that other particles are ``transparent" to them.  
